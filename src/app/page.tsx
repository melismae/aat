...
const Content = [
  ...
  },
  {
    ...
    text: (<><h3>Think I might be a good fit for you?</h3><p className="mb-0">Get in touch for a free 15 minute consultation.</p><p className="mt-0">Call <a className="link link-hover" href="tel:+9713596158">971.359.6158</a> | Email <a className="link link-hover" href="mailto:aaron@kelledebruin.com">aaron@kelledebruin.com</a></p>
      ...
      <div className="mt-0"><Link className="btn btn-outline btn-primary" href="https://kelledebruin.com/client-portal-contact-information/" target="_blank">Request an appointment</Link></div>
    ...
    <main className="flex min-h-screen flex-col">
      ...
      <section className="">
        <div className="prose w-full m-auto pt-16 pb-24 text-center">
          ...
          <div>
            <p className="mt-1">Currently accepting new teletheraphy clients in Oregon.</p>
            <Link className="btn mr-8" href="https://kelledebruin.com/client-portal-contact-information/" target="_blank">Request an appointment</Link>
            <Link className="btn" href="https://kelledebruin.com/client-portal-contact-information/" target="_blank">Schedule a free consultation </Link></div></div>
      </section>
      {Content.map((item, index) => {
        if (item.image) {
          return (
            <section className="flex flex-row" id={item.anchor}>
              <div className="prose basis-1/3">
                {item.image}
              </div>
              <div className="prose basis-2/3 p-24">
                <h1>{item.title}</h1>
                {item.text}
              </div>
            </section>
        ...
        return (
          <section className={`${item.background} flex flex-row py-24 px-36`} key={index} id={item.anchor}>
            ...
            <div className="prose basis-2/3 px-24"><div className="p-2 bg-primary-content rounded-xl">{item.text}</div></div>
          </section>
        )
      })}
    </main >
